...
\item \texttt{-noJsDirect}: By default, client-side functions whose bodies never need to suspend (no \texttt{rpc}, \texttt{recv}, or \texttt{sleep}) are compiled to plain JavaScript functions, while the remaining code is shipped as an AST for the interpreter in \texttt{urweb.js}.  This flag disables the direct compilation, so that all client-side code is interpreted.  It is mainly useful for isolating bugs in the JavaScript code generator.
