...
To access the SML-NJ build of the Ur/Web compiler via Nix (including some convenience wrappers), run:
\begin{verbatim}
$ nix-shell
[nix-shell]$ repl
- c "tests/alert"; (* compile this Ur/Web project *)
- r (); (* rebuild the Ur/Web compiler itself *)
\end{verbatim}
...
Here is the complete list of directive forms.  ``FFI'' stands for ``foreign function interface,'' Ur's facility for interaction between Ur programs and C and JavaScript libraries.  Note that options set on the command-line override the value set by the directive.
\begin{itemize}
...
\item \texttt{database DBSTRING} sets the connection string for the database backend.  The format is the one expected by Postgres's \texttt{PQconnectdb()}, described under the \texttt{-db} command-line option in Section \ref{sec:building}.
\item \texttt{dbms [postgres|mysql|sqlite]} selects the database backend (the default is \texttt{postgres}).
\item \texttt{debug} compiles the generated C code without optimization and with debugging symbols (\texttt{-O0 -g}), keeps that code in \texttt{/tmp/webapp.c} (and its object file in \texttt{/tmp/webapp.o}).  This is mostly useful to help in debugging the compiler itself; the same effect is available via the \texttt{-debug} command-line option.
\item \texttt{dev} compiles the generated C code without optimization (\texttt{-O0}), which makes builds noticeably faster during development, without the other effects of \texttt{debug}.  The same effect is available as the \texttt{-dev} command-line option.
...
\item \texttt{lessSafeFfi} enables \texttt{ffi} declarations inside \texttt{.ur} files, so that FFI functions can be declared next to the code that uses them rather than in a separate \texttt{.urs} file named by an \texttt{ffi} directive.  This weakens a useful property (that only \texttt{.urp} files need auditing to trust an application's use of the FFI), so it must be requested explicitly; see Section \ref{sec:lessSafeFfi}.
...
\item \texttt{library FILENAME} parses \texttt{FILENAME.urp} and merges its contents with the rest of the current file's contents.  If \texttt{FILENAME.urp} doesn't exist, the compiler also tries \texttt{FILENAME/lib.urp}.    Directives that configure the deployment of an application are ignored in a library, with a warning.
...
\item \texttt{mimeTypes PATH} sets the name of the file from which the MIME-type database is read, as a substitute for the usual \texttt{/etc/mime.types} on UNIX systems.  It should come before any \texttt{file} directive, since those look up MIME types as they are read.
...
\item \texttt{noMangleSql} avoids adding a \texttt{uw\_} prefix in front of each identifier in SQL.  An identifier is quoted (with the database backend's identifier delimiter) only when it is one of that backend's reserved words (e.g. a table called \texttt{order} or a field called \texttt{Select}) or when it could not otherwise be an identifier (e.g. the column \texttt{1} that stands for an anonymous \texttt{SELECT} expression).
...
\item \texttt{path NAME=VALUE} creates a mapping from \texttt{NAME} to \texttt{VALUE}.  This mapping may be used at the beginnings of filesystem paths given to various other configuration directives.  A path like \texttt{\$NAME/rest} is expanded to \texttt{VALUE/rest}.  There is an initial mapping from the empty name (for paths like \texttt{\$/list}) to the directory where the Ur/Web standard library is installed.  If you accept the default \texttt{configure} options, this directory is \texttt{/usr/local/lib/urweb/ur}.  The mappings also determine how compiler messages, \texttt{\_LOC\_}, and the messages of \texttt{error} name source files: a file below \texttt{VALUE} is named \texttt{\$NAME/rest}, with the longest matching \texttt{VALUE} winning; a file below the directory the compiler runs in, but not below any \texttt{VALUE}, is named relative to that directory; any other file is named by its absolute path.
...
\item \texttt{-debug}: Same as the \texttt{debug} directive in a project file: compile the generated C code with \texttt{-O0 -g}, keep it in \texttt{/tmp/webapp.c}, and print the compiler phases as they run.
\item \texttt{-dev}: Same as the \texttt{dev} directive in a project file: compile the generated C code without optimization, for faster builds.
...
\item \texttt{-dumpVerboseSource}: Like \texttt{-dumpSource}, but prints the intermediate program with more detail (e.g., the internal numbering of identifiers), which can help when reading the output of the later, lower-level phases.
...
\item \texttt{-help}, \texttt{-version}, \texttt{-numeric-version}: Print, respectively, a summary of the command-line options, the version of the compiler, and its version as a plain number (convenient for scripts); then exit.
...
\item \texttt{-js NAME}: Ur/Web applications with client-side code link in generated JavaScript files, which, by default, are assigned content-hash-based names.  There are two such files: the Ur/Web runtime, which is the same for every application built with a given version of the compiler, and the application's own code, which is loaded after it.  Use this option to override the filename for the application's code; the runtime keeps its default, has-based name.  The name is the last component of the JS's URL, directly below the application prefix; it must not contain \texttt{/}.  The code is embedded in the executable.  Be forewarned that the default method uses a name based on hashing the code itself, which is done for a good reason: browsers are very eager to cache JavaScript code, and application changes may fail to propagate quickly to browsers if this filename stays the same between versions.  In such cases, it isn't just that the user sees an old version of your application.  Instead, the application runs with a mix of old and new files, leading to arbitrary bugs that Ur/Web prevents, when used properly.
...
\item \texttt{-sql FILENAME}: Set where a database set-up SQL script is written.  Run with \texttt{-stop sqlify} if all you want is this file.

\item \texttt{-static}: Link the runtime system statically.  The default is to link against dynamic libraries.

\item \texttt{-stop PHASE}: Stop compilation after the named phase, print the intermediate program to stderr and exit nonzero.  This flag is mainly useful for debugging the Ur/Web compiler itself.

\item \texttt{-stopQuiet PHASE}: Like \texttt{-stop}, but doesn't print the intermediate output and exits zero.
\end{itemize}
...
\begin{itemize}
  ...
  \item \cd{URWEB\_SQLITE\_DB\_PATH}: when using SQLite, overrides the compiled-in database path
  \item \cd{URWEB\_DML\_LOG}: (SQLite only, currently) upon a successful transaction, appends executed DML statements to the given file. The file is opened and closed in every transaction commit, so if it is (re)moved the next transaction creates a new one. NB: Be aware that you may be logging sensitive information, so use responsibly in production
  \item \cd{URWEB\_SIGFILE\_PATH}: overrides the compiled-in path of the signing key (the \texttt{sigfile} directive), or sets one for an application compiled without
  \item \cd{URWEB\_REPRODUCIBLE\_EPOCH}: for tests, pins the clock that \cd{now} and \cd{CURRENT\_TIMESTAMP} read to the given number of seconds since the epoch; the first request is served at that time and every further request one second later
  \item \cd{URWEB\_REPRODUCIBLE\_PRNG\_SEED}: for tests, makes \cd{rand} and the passwords of clients pseudo-random numbers that depend on nothing but the given unsigned 64-bit integer, on how many requests arrived before the one that draws them. (NB: so never set this in production, since it makes those passwords predictable!)
\end{itemize}
...
The current page can be reloaded, as with the browser's reload button.  As for any navigation started from a script, the page is only torn down once the calling thread yields, so code following the call may still run.
$$\begin{array}{l}
  \mt{val} \; \mt{reload} : \mt{transaction} \; \mt{unit}
\end{array}$$
...
$$\begin{array}{rrcll}
  ...
  &&& (\mt{INSERT} \; \mt{INTO} \; T^E \; \mt{SET} \; (f = E,)^+) & \textrm{(Nonstandard syntax extension)} \\
