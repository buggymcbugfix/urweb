...
fun fromList [a] (ls : list a) : t a =
    case ls of
        [] => error <xml>Nelist.fromList: the list is empty</xml>
      | x :: ls => {First = x, Rest = ls}

fun fromListResult [a] (ls : list a) : result (t a) =
    case ls of
        [] => Failure <xml>Nelist.fromListResult: the list is empty</xml>
...
fun mapM [m] (_ : monad m) [a] [b] (f : a -> m b) (nel : t a) : m (t b) =
    first <- f nel.First;
    rest <- List.mapM f nel.Rest;
    return {First = first, Rest = rest}

fun mapX [a] [ctx] (f : a -> xml ctx [] []) (nel : t a) : xml ctx [] [] =
    <xml>{f nel.First}{List.mapX f nel.Rest}</xml>

fun mem [a] (_ : eq a) (x : a) (nel : t a) : bool =
    nel.First = x || List.mem x nel.Rest
...
fun search [a] [b] (f : a -> option b) (nel : t a) : option b =
    case f nel.First of
        None => List.search f nel.Rest
      | v => v

fun rev [a] (nel : t a) : t a =
    case List.rev nel.Rest of
        [] => nel
