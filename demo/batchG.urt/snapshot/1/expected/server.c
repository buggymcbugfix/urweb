#include "include/urweb/config.h"
 ...
 static void uw_setup_limits() {
  ...
  void uw_global_custom() {
   ...
   static void uw_db_validate(uw_context ctx) {
    ...
    res = PQexec(conn, "SELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'uw_batchg_t'");
     ...
     char msg[1024];
      ...
      PQclear(res);
      PQfinish(conn);
      uw_error(ctx, FATAL, "Query failed:\nSELECT COUNT(*) FROM information_schema.tables WHERE table_name = 'uw_batchg_t'\n%s", msg);
      }
     ...
     PQclear(res);
     res = PQexec(conn, "SELECT COUNT(*) FROM information_schema.columns WHERE table_name = 'uw_batchg_t' AND ((LOWER(column_name) = 'uw_id' AND data_type IN ('bigint', 'numeric', 'integer') AND is_nullable = 'NO') OR (LOWER(column_name) = 'uw_a' AND data_type IN ('text', 'character varying') AND is_nullable = 'NO') OR (LOWER(column_name) = 'uw_b' AND data_type = 'double precision' AND is_nullable = 'NO'))");
     ...
     char msg[1024];
      ...
      PQclear(res);
      PQfinish(conn);
      uw_error(ctx, FATAL, "Query failed:\nSELECT COUNT(*) FROM information_schema.columns WHERE table_name = 'uw_batchg_t' AND ((LOWER(column_name) = 'uw_id' AND data_type IN ('bigint', 'numeric', 'integer') AND is_nullable = 'NO') OR (LOWER(column_name) = 'uw_a' AND data_type IN ('text', 'character varying') AND is_nullable = 'NO') OR (LOWER(column_name) = 'uw_b' AND data_type = 'double precision' AND is_nullable = 'NO'))\n%s", msg);
      }
     
     if (strcmp(PQgetvalue(res, 0, 0), "3")) {
     PQclear(res);
      PQfinish(conn);
      uw_error(ctx, FATAL, "Table 'uw_batchg_t' has the wrong column types.");
      }
     
     PQclear(res);
     
     res = PQexec(conn, "SELECT COUNT(*) FROM information_schema.columns WHERE table_name = 'uw_batchg_t' AND LOWER(column_name) LIKE 'uw_%'");
     ...
     char msg[1024];
      ...
      PQclear(res);
      PQfinish(conn);
      uw_error(ctx, FATAL, "Query failed:\nSELECT COUNT(*) FROM information_schema.columns WHERE table_name = 'uw_batchg_t' AND LOWER(column_name) LIKE 'uw_%'\n%s", msg);
      }
     
     if (strcmp(PQgetvalue(res, 0, 0), "3")) {
     PQclear(res);
      PQfinish(conn);
      uw_error(ctx, FATAL, "Table 'uw_batchg_t' has extra columns.");
      }
    ...
    res = PQprepare(conn, "uw0", "SELECT T_T.uw_A, T_T.uw_B, T_T.uw_Id FROM uw_BatchG_t AS T_T", 0, NULL);
     ...
     char msg[1024];
      ...
      PQclear(res);
      PQfinish(conn);
      uw_error(ctx, FATAL, "Unable to create prepared statement:\nSELECT T_T.uw_A, T_T.uw_B, T_T.uw_Id FROM uw_BatchG_t AS T_T\n%s", msg);
      }
     PQclear(res);
     
     
     res = PQprepare(conn, "uw1", "INSERT INTO uw_BatchG_t (uw_A, uw_B, uw_Id) VALUES ($1::text, $2::float8, $3::int8)", 0, NULL);
      if (PQresultStatus(res) != PGRES_COMMAND_OK) {
      char msg[1024];
       strncpy(msg, PQerrorMessage(conn), 1024);
       msg[1023] = 0;
       PQclear(res);
       PQfinish(conn);
       uw_error(ctx, FATAL, "Unable to create prepared statement:\nINSERT INTO uw_BatchG_t (uw_A, uw_B, uw_Id) VALUES ($1::text, $2::float8, $3::int8)\n%s", msg);
       }
      PQclear(res);
      
     
     res = PQprepare(conn, "uw2", "DELETE FROM uw_BatchG_t AS T_T WHERE (T_T.uw_Id = $1::int8)", 0, NULL);
      if (PQresultStatus(res) != PGRES_COMMAND_OK) {
      char msg[1024];
       strncpy(msg, PQerrorMessage(conn), 1024);
       msg[1023] = 0;
       PQclear(res);
       PQfinish(conn);
       uw_error(ctx, FATAL, "Unable to create prepared statement:\nDELETE FROM uw_BatchG_t AS T_T WHERE (T_T.uw_Id = $1::int8)\n%s", msg);
       }
      PQclear(res);
      }
 ...
 /* No global setup for LRU cache. */
  ...
  /* SQL table uw_BatchG_t keys uw_Id constraints   */
            ...
            PGconn *conn = uw_get_db(ctx);
             ...
             int n, i;
              ...
              if (!strcmp_nullsafe(PQresultErrorField(res, PG_DIAG_SQLSTATE), "40001")) {
               ...
               PQclear(res);
               uw_error(ctx, FATAL, "demo/batchFun.ur:42:8-42:13: Query failed:\n%s\n%s", 
               "SELECT T_T.uw_A, T_T.uw_B, T_T.uw_Id FROM uw_BatchG_t AS T_T", PQerrorMessage(conn));
               }
              
              if (PQnfields(res) != 3) {
              int nf = PQnfields(res);
               PQclear(res);
               uw_error(ctx, FATAL, "demo/batchFun.ur:42:8-42:13: Query returned %d columns instead of 3:\n%s\n%s", nf, 
               "SELECT T_T.uw_A, T_T.uw_B, T_T.uw_Id FROM uw_BatchG_t AS T_T", PQerrorMessage(conn));
               }
              ...
              struct __uws_3 __uwr_r_2;
               ...
               __uwr_r_2.__uwf_T.__uwf_A =
                (PQgetisnull(res, i, 0) ? ({uw_Basis_string
                                           tmp;
                                           uw_error(ctx, FATAL, "demo/batchFun.ur:42:8-42:13: Unexpectedly NULL field #0");
                                           tmp;
                                           }) : uw_strdup(ctx, PQgetvalue(res, i, 0)));
                
                __uwr_r_2.__uwf_T.__uwf_B =
                 (PQgetisnull(res, i, 1) ? ({uw_Basis_float
                                            tmp;
                                            uw_error(ctx, FATAL, "demo/batchFun.ur:42:8-42:13: Unexpectedly NULL field #1");
                                            tmp;
                                            }) : uw_Basis_stringToFloat_error(ctx, 
                                                  PQgetvalue(res, i, 1)));
                 
                __uwr_r_2.__uwf_T.__uwf_Id =
                 (PQgetisnull(res, i, 2) ? ({uw_Basis_int
                                            tmp;
                                            uw_error(ctx, FATAL, "demo/batchFun.ur:42:8-42:13: Unexpectedly NULL field #2");
                                            tmp;
                ...
                *tmp =
                ({ struct __uws_2 tmp = {__uwr_r_2.__uwf_T, __uwr_acc_3}; tmp;
                 });
                tmp;
                });
               }
            ...
            disc == NULL ? 0
             ...
             disc != NULL && 1 && 1 ?
              ({struct __uws_1 __uwr_r_2 = (*disc).__uwf_1;
                 struct __uws_2* __uwr_lsPRIME_3 = (*disc).__uwf_2;
                  ({
                                            ...
                                            PGconn *conn = uw_get_db(ctx);
                                             static const int paramFormats[] = { 
                                              ...
                                              paramValues[0] = arg1;
                                               ...
                                               paramValues[1] = uw_Basis_attrifyFloat(ctx, 
                                                                 arg2);
                                             ...
                                             res = PQexecPrepared(conn, "uw1", 3, paramValues, paramLengths, paramFormats, 0);
                                             ...
                                             if (res == NULL) {
                                              ...
                                              if (!strcmp_nullsafe(PQresultErrorField(res, PG_DIAG_SQLSTATE), "40001")) {
                                               ...
                                               PQclear(res);
                                                uw_error(ctx, FATAL, "demo/batchFun.ur:59:12-60:23: DML failed:\n%s\n%s", 
                                                "INSERT INTO uw_BatchG_t (uw_A, uw_B, uw_Id) VALUES ($1::text, $2::float8, $3::int8)", PQerrorMessage(conn));
                                               }
                   ...
                   uw_end_region(ctx);
                    ({
                     struct __uws_2* rearg0 = __uwr_lsPRIME_3;
                      uw_unit rearg1 = 0;
                     uw_unit
                     tmp;
                     __uwr_ls_0 = rearg0;
              ...
              ({
               uw_unit
               tmp;
               uw_error(ctx, FATAL, "demo/batchFun.ur:55:4-62:4: pattern match failure");
               tmp;
               });
            }));
    }
             ...
             if (res == NULL) {
              ...
              if (!strcmp_nullsafe(PQresultErrorField(res, PG_DIAG_SQLSTATE), "40001")) {
               ...
               PQclear(res);
                ...
                "DELETE FROM uw_BatchG_t AS T_T WHERE (T_T.uw_Id = $1::int8)", PQerrorMessage(conn));
               }
             ...
             disc == uw_Basis_True ? (uw_write(ctx, "true"), 0)
              ...
              disc == uw_Basis_False ? (uw_write(ctx, "false"), 0)
               ...
               ({
                uw_unit
                tmp;
                uw_error(ctx, FATAL, "demo/batchFun.ur:85:30-94:30: pattern match failure");
                tmp;
                });
             }),
            ...
            disc == NULL ? "null"
             ...
             disc != NULL && 1 ?
              ({struct __uws_2 __uwr_x_1 = (*disc);
                 ({
                  uw_Basis_string arg0 = "{_1:{_A:";
                   
                   uw_Basis_string arg1 =
                    uw_Basis_jsifyString(ctx, __uwr_x_1.__uwf_1.__uwf_A);
                    uw_Basis_string arg2 = ",_B:";
                     
                   uw_Basis_string arg3 =
                    uw_Basis_htmlifyFloat(ctx, __uwr_x_1.__uwf_1.__uwf_B);
                    uw_Basis_string arg4 = ",_Id:";
                     
                   uw_Basis_string arg5 =
                    uw_Basis_htmlifyInt(ctx, __uwr_x_1.__uwf_1.__uwf_Id);
              ...
              ({
               uw_Basis_string
               tmp;
               uw_error(ctx, FATAL, "demo/batchFun.ur:135:78-135:81: pattern match failure");
               tmp;
               });
            }));
    }
                                     ...
                                     uw_end_region(ctx), ((uw_write(ctx, ">\n<h2>Rows</h2>\n"), 0),
                                                          (uw_begin_region(ctx),
                                                           ...
                                                           uw_end_region(ctx), ((uw_write(ctx, 
                                                                               ...
                                                                               (uw_begin_region(ctx),
                                                                               ((uw_write(ctx, 
                                                                               "{c:\"a\",f:{c:\"a\",f:{c:\"n\",n:5},x:{c:\"c\",v:{_A:"), 0),
                                                                               (uw_begin_region(ctx),
                                                                               uw_Basis_htmlifySource_w(ctx,
                                                                               __uwr_inps_5.__uwf_A
                                                                               ),
                                                                               uw_end_region(ctx),
                                                                               ((uw_write(ctx, 
                                                                               ",_B:"), 0),
                                                                               ...
                                                                               uw_end_region(ctx),
                                                                               ((uw_write(ctx, 
                                                                               "));</script></td> </tr><tr> <th>B:</th> <td><script type=\"text/javascript\">var d=inp(exec("), 0),
                                                                               (uw_begin_region(ctx),
                                                                               ((uw_write(ctx, 
                                                                               "{c:\"a\",f:{c:\"a\",f:{c:\"n\",n:6},x:{c:\"c\",v:{_A:"), 0),
                                                                               (uw_begin_region(ctx),
                                                                               uw_Basis_htmlifySource_w(ctx,
                                                                               __uwr_inps_5.__uwf_A
                                                                               ),
                                                                               ...
                                                                               (uw_begin_region(ctx),
                                                                               ((uw_write(ctx, 
                                                                               "{c:\"a\",f:{c:\"a\",f:{c:\"a\",f:{c:\"a\",f:{c:\"n\",n:7},x:{c:\"c\",v:"), 0),
                                                                               (uw_begin_region(ctx),
                                                                               uw_Basis_htmlifySource_w(ctx,
                                                                               __uwr_batched_3
                                                                               ),
