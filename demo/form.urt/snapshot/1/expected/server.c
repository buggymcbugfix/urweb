#include "include/urweb/config.h"
  ...
  struct __uws_1
   ...
   uw_Basis_string __uwf_A;
    uw_Basis_string __uwf_B;
     uw_Basis_bool __uwf_C;
      };
  
  static uw_unit
   __uwn_wrap_handler_1662(uw_context ctx, struct __uws_1 __uwr_x0_0, 
                            uw_unit __uwr___1)
   {
 ...
 if (!strncmp(request, "/Form/main", 10) && (request[10] == 0 || request[10] == '/')) {
  request += 10;
  if (*request == '/') ++request;
  ...
  if (!strncmp(request, "/Form/handler", 13) && (request[13] == 0 || request[13] == '/')) {
   request += 13;
   if (*request == '/') ++request;
   uw_write_header(ctx, "Content-type: text/html; charset=utf-8\r\n");
    uw_write(ctx, uw_begin_html5);
    ...
    uw_Basis_string uw_input_A;
     ...
     request = uw_get_input(ctx, 0);
      ...
      request = uw_get_input(ctx, 1);
       ...
       request = uw_get_optional_input(ctx, 2);
        if (request == NULL)
        uw_error(ctx, FATAL, "Missing input C");
        uw_input_C = uw_Basis_unurlifyBool(ctx, &request);
        struct __uws_1 uw_inputs = {
         uw_input_A,
          uw_input_B,
           uw_input_C,
            };
     __uwn_wrap_handler_1662(ctx, uw_inputs, 0);
    uw_write(ctx, "</html>");
     return;
    }
    }
