#include "include/urweb/config.h"
 ...
 /* No global setup for LRU cache. */
  ...
  static uw_unit
   __uwn_wrap_target_1662(uw_context ctx, uw_unit __uwr_x0_0, uw_unit __uwr___1)
   {
 ...
 if (!strncmp(request, "/Link/main", 10) && (request[10] == 0 || request[10] == '/')) {
  request += 10;
  if (*request == '/') ++request;
  ...
  if (!strncmp(request, "/Link/target", 12) && (request[12] == 0 || request[12] == '/')) {
   request += 12;
   if (*request == '/') ++request;
   uw_write_header(ctx, "Content-type: text/html; charset=utf-8\r\n");
    uw_write(ctx, uw_begin_html5);
   ...
   {
    uw_unit arg0 = uw_Basis_unurlifyUnit(ctx, &request);
     __uwn_wrap_target_1662(ctx, arg0, 0);
    uw_write(ctx, "</html>");
     return;
    }
    }
