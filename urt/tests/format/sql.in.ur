...
fun active' () =
	queryL1 (SELECT tbUser.Id, tbUser.Name FROM tbUser
		WHERE tbUser.Active)
...
fun joins () =
	queryL1
		(
			...
			FROM tbUser AS U
			JOIN tbOrder AS O ON O.UserId = U.Id
			INNER JOIN tbOrder AS O2 ON O2.UserId = U.Id
			LEFT JOIN tbOrder AS O3 ON O3.UserId = U.Id
			LEFT OUTER JOIN tbOrder AS O4 ON O4.UserId = U.Id
			RIGHT JOIN tbOrder AS O5 ON O5.UserId = U.Id
			RIGHT OUTER JOIN tbOrder AS O6 ON O6.UserId = U.Id
			FULL JOIN tbOrder AS O7 ON O7.UserId = U.Id
			FULL OUTER JOIN tbOrder AS O8 ON O8.UserId = U.Id
			LEFT JOIN
				(
