...
con colMeta =
	fn (row :: {Type}) (global_input_filter :: (Type * Type * Type)) =>
		{
...
structure Direct =
	struct
		con metaBase =
			fn actual_input_filter :: (Type * Type * Type) =>
				{
		...
		con meta =
			fn global_actual_input_filter =>
				{
					Initialize : transaction global_actual_input_filter.1,
					Handlers :
						global_actual_input_filter.1 ->
		...
		fun editable [ts] [rest] [nm :: Name] [[nm] ~ rest] name (m : meta ts) :
			colMeta
				([nm = ts.2] ++ rest)
				(editableState ts)
		=
		...
		fun readOnly [ts] [rest] [nm :: Name] [[nm] ~ rest] name (m : meta ts) :
			colMeta
				([nm = ts.2] ++ rest)
				(readOnlyState ts)
		=
