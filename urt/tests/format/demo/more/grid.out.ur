con colMeta' =
	fn (row :: Type) (input :: Type) (filter :: Type) =>
		{
			Header : string,
...
con colMeta =
	fn (row :: Type) (global :: Type, input :: Type, filter :: Type) =>
		{
...
=
	struct
		...
		fun makeAll cols row =
			@@Monad.exec [transaction] _
				[map snd3 M.cols]
				(
					@map2 [fst3]
						[colMeta M.row]
						[fn p => transaction (snd3 p)]
			...
			<xml>
				<table class={tab}>
										...
										<xml><th class={header}>
											{
												...
												| sort =>
													<xml><button
														value={(meta.Handlers data).Header}
														onclick={fn _ => set grid.Sort sort}
													/></xml>
											}
										</th></xml>
								)
...

					{
						Dlist.render
							(
								fn {Row = rowS, Cols = colsS, Updating = ud, Selected = sd} pos =>
									let
										val delete =
											Dlist.delete pos;
											row <- get rowS;
										...
										val save =
											...
											| Some s =>
												alert
													(
														"Can't save because the following columns have invalid values:\n"
															^ s
													)
											| None =>
												set ud False;
							...
							@mapX3
								...
								(
									fn
										...
										state
										filter
									=> <xml><td>{(meta.Handlers state).DisplayFilter filter}</td></xml>
								)
								M.folder
								M.cols
								grid.Cols
