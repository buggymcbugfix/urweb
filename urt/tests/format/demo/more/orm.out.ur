...
=
	struct
		...
		val id =
			{
				Link = fn id => resultOut (SELECT * FROM t WHERE t.Id = {[id]}),
				Inj = inj
			}

		fun ensql [avail ::_] (r : row') : $(map (sql_exp avail [] []) fs') =
			@map2 [meta] [fst]
				[fn ts :: (Type * Type) => sql_exp avail [] [] ts.1]
		...
		con meta' =
			fn (fs :: {Type}) (col :: Type, parent :: Type) =>
				{
					Col :
						{
							Exp : sql_exp [T = fs] [] [] col,
							Inj : sql_injectable col
						},
